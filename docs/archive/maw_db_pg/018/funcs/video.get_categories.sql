...
DROP FUNCTION IF EXISTS video.get_categories(TEXT[], SMALLINT, SMALLINT, SMALLINT);
DROP FUNCTION IF EXISTS video.get_categories(TEXT[], SMALLINT, SMALLINT, SMALLINT, SMALLINT);
...
(
    ...
    _video_id SMALLINT DEFAULT NULL
)
RETURNS TABLE
(
    id SMALLINT,
...
BEGIN
    IF _video_id IS NOT NULL THEN
        SELECT v.category_id INTO _catid
          FROM video.video v
         WHERE v.id = _video_id
           AND (_id IS NULL OR _id = v.category_id);
