...
function run_psql_script() {
    if [ "${PODNAME}" == "" ]; then
        ...
        podman run -it --rm --pod "${PODNAME}" --env-file "${ENVFILE}" -v "$(pwd)":/tmp/context:ro --security-opt label=disable postgres:12.2 psql -h 127.0.0.1 -U postgres -d idsrv -q -f "/tmp/context/${1}"
    fi
}
