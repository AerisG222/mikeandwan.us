...
namespace MawMvcApp.ViewModels.Gps
{
		...
		: IValidatableObject
	{
		...
		public void Convert()
		{
			...
			DLatString = $"{Math.Abs(degrees):f4} deg {coord.LatitudeRef.ToString()}";

			coord.GetLongitudeDegrees(out degrees);
			DLngDegrees = Math.Abs(degrees);
			DLngReference = coord.LongitudeRef;
			DLngString = $"{Math.Abs(degrees):f4} deg {coord.LongitudeRef.ToString()}";
			...
			DmLatString = $"{Math.Abs(degrees)} deg {minutes:f4}' {coord.LatitudeRef.ToString()}";
			...
			DmLngString = $"{Math.Abs(degrees)} deg {minutes:f4}' {coord.LongitudeRef.ToString()}";
			...
			DmsLatString = $"{Math.Abs(degrees)} deg {minutes}' {seconds:f4}\" {coord.LatitudeRef.ToString()}";
			...
			DmsLngString = $"{Math.Abs(degrees)} deg {minutes}' {seconds:f4}\" {coord.LongitudeRef.ToString()}";
		}
