...
import { BoardSector } from '../models/BoardSector';

@Component({
    selector: 'spinner',	
    templateUrl: '/js/games/money_spin/components/Spinner.html'
})
export class Spinner implements OnInit, OnDestroy {
    private static SECTORS = [ new BoardSector(37,  8),
    ...
    spin() : void {
        ...
        this._arrowSpeed = Spinner.MIN_TOP_SPEED + (Math.random() * Spinner.MAX_ADDITIONAL_TOP_SPEED);
        this._speedDropoff = Spinner.MIN_DROPOFF + (Math.random() * Spinner.MAX_ADDITIONAL_DROPOFF);
        let fullSpeedTime = Spinner.MIN_TOP_SPEED_TIME_MS + (Math.random() * Spinner.MAX_ADDITIONAL_TOP_SPEED_TIME_MS);
    ...
    private getScore(deg : number) : number {
        for(let i = 0; i < Spinner.SECTORS.length; i++) {
            let score = Spinner.SECTORS[i];
