...
import { ISelectedCards } from '../interfaces/ISelectedCards';
import { MemoryService } from '../services/MemoryService';

@Component({
    selector: 'game-board',
    directives: [ Card, NgClass, NgFor ],
    templateUrl: '/js/games/memory/components/GameBoard.html'
})
export class GameBoard {
    ...
    private _matchedCards : Array<Card> = [];
    ...
    evaluateTurn() : void {
        if(this._selectedCards.card1.cardInfo.id === this._selectedCards.card2.cardInfo.id) {
            this.removeCards();
            this.match.next(this._matchedCards.length === GameBoard.CARDS_IN_GAME);
        }
        else {
            this.unFlipCards();
